...
int main(int argc, char* argv[]){
  if(argc!=2){
	cout << "Usage is makeLED and one of: " << endl;
        cout << "   on, off, toggle, flash or status" << endl;
   ...
   if (cmd == "toggle")
   {
	//Getting LED Status
	std::fstream fs_LED;
        fs_LED.open( LED3_PATH "/brightness", std::fstream::in);
        string line;
        while(getline(fs_LED,line))
	{       
		cout <<"LED current status is " <<  line << endl;
